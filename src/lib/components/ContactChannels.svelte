<script>
	import { contact, whatsappLink } from '$lib/site.js';
</script>

<!-- Canais de contato com ícones — usado na home (#contato) e no ContactSection
     das páginas de blog, para os dois blocos nunca divergirem. -->
<ul class="mt-6 space-y-3 text-sm text-slate-700 sm:text-base">
	<li class="flex items-center gap-2.5">
		<svg class="h-5 w-5 shrink-0 text-tim-600" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"
			><path
				d="M6.6 10.8a15.5 15.5 0 006.6 6.6l2.2-2.2c.3-.3.7-.4 1-.2 1.1.4 2.3.6 3.6.6.6 0 1 .4 1 1V20c0 .6-.4 1-1 1A17 17 0 013 4c0-.6.4-1 1-1h3.4c.6 0 1 .4 1 1 0 1.3.2 2.5.6 3.6.1.4 0 .8-.2 1l-2.2 2.2z"
			/></svg
		>
		<span>
			Telefone:
			<a class="font-semibold text-tim-700 hover:underline" href={contact.phoneHref}>
				{contact.phoneLabel}
			</a>
		</span>
	</li>
	<li class="flex items-center gap-2.5">
		<svg class="h-5 w-5 shrink-0 text-tim-600" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"
			><path
				d="M12.04 2C6.58 2 2.13 6.45 2.13 11.91c0 1.75.46 3.45 1.32 4.95L2 22l5.25-1.38a9.9 9.9 0 0 0 4.79 1.22h.01c5.46 0 9.91-4.45 9.91-9.91S17.5 2 12.04 2zm0 18.15h-.01a8.2 8.2 0 0 1-4.19-1.15l-.3-.18-3.11.82.83-3.03-.2-.31a8.24 8.24 0 0 1-1.26-4.39c0-4.54 3.7-8.24 8.25-8.24 2.2 0 4.27.86 5.82 2.42a8.18 8.18 0 0 1 2.41 5.83c0 4.54-3.7 8.24-8.24 8.24zm4.52-6.16c-.25-.12-1.47-.72-1.69-.81-.23-.08-.39-.12-.56.13-.16.25-.64.81-.79.97-.14.17-.29.19-.54.06-.25-.12-1.05-.39-1.99-1.23-.74-.66-1.23-1.47-1.38-1.72-.14-.25-.02-.38.11-.51.11-.11.25-.29.37-.43.13-.14.17-.25.25-.41.08-.17.04-.31-.02-.43-.06-.12-.56-1.34-.76-1.84-.2-.48-.4-.42-.56-.43-.14-.01-.31-.01-.48-.01a.92.92 0 0 0-.66.31c-.23.25-.87.85-.87 2.07 0 1.22.89 2.4 1.01 2.56.12.17 1.75 2.67 4.23 3.74.59.26 1.05.41 1.41.52.59.19 1.13.16 1.56.1.48-.07 1.47-.6 1.68-1.18.21-.58.21-1.07.14-1.18-.06-.11-.22-.17-.47-.29z"
			/></svg
		>
		<span>
			WhatsApp:
			<a
				class="font-semibold text-tim-700 hover:underline"
				target="_blank"
				rel="noopener"
				href={whatsappLink()}
			>
				{contact.whatsappLabel}
			</a>
		</span>
	</li>
	<li class="flex items-center gap-2.5">
		<svg class="h-5 w-5 shrink-0 text-tim-600" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"
			><path
				d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"
			/></svg
		>
		<span>
			E-mail:
			<a class="font-semibold text-tim-700 hover:underline" href="mailto:{contact.emails[0]}">
				{contact.emails[0]}
			</a>
		</span>
	</li>
	<li class="flex items-start gap-2.5">
		<svg
			class="mt-0.5 h-5 w-5 shrink-0 text-tim-600"
			viewBox="0 0 24 24"
			fill="currentColor"
			aria-hidden="true"
			><path
				d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5S10.62 6.5 12 6.5s2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"
			/></svg
		>
		<span>{contact.address}</span>
	</li>
</ul>
